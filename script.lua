--==============================================================
--  CONES SCRIPT  v5  ·  no hooks · profiles · debug
--  Ключ: TimohaGay · Меню: P · Watermark: O
--==============================================================
...
local AUTH_FILE = "cones/auth.txt"
...
local S = {
    ...
    -- клавиши
    KeyMenu = "P", KeyWM = "O", KeyAim = "MouseButton2",
    KeyFly = "F", KeyNoclip = "N", KeyClickTP = "T",
    KeyPanic = "End", KeyQuick = "K",
}
...
local function Keybind(page, tab, name, key)
    ...
    local function refresh() kt.Text = tostring(S[key]):upper() end
    refresh()
    local hit = Instance.new("TextButton", f)
    hit.Size = UDim2.fromScale(1, 1); hit.BackgroundTransparency = 1
    hit.Text = ""; hit.AutoButtonColor = false
    Maid:Conn(hit.MouseButton1Click:Connect(function()
        ...
        conn = UIS.InputBegan:Connect(function(i)
            ...
            if i.UserInputType == Enum.UserInputType.Keyboard then n = i.KeyCode.Name
            elseif i.UserInputType.Name:find("MouseButton") then n = i.UserInputType.Name end
            if n then S[key] = n; refresh(); conn:Disconnect() end
        end)
    end))
    register(f, name, tab, refresh)
end
...
Maid:Conn(RunService.RenderStepped:Connect(function(dt)
    ...
    fovCircle.Visible = S.ShowFOV and S.AimOn
...
Maid:Conn(UIS.InputBegan:Connect(function(input, processed)
    ...
    elseif keyMatches(input, S.KeyPanic) then
        panic()
        print("[CONES] PANIC — всё выключено, " .. tostring(S.KeyMenu) .. " — вернуть меню")
    end
end))
...
ColorPicker(pEsp, "ESP", "Цвет друга", "ColFriend")
...
Button(pSet, "Config", "Забыть ключ", function()
    if delfile and isfile and isfile(AUTH_FILE) then
        pcall(delfile, AUTH_FILE)
        notify("КЛЮЧ ЗАБЫТ — СПРОСИТ В СЛЕДУЮЩИЙ РАЗ")
    else
        notify("ФАЙЛ КЛЮЧА НЕ НАЙДЕН")
    end
end)
...
applyConfig()
notify("Cones v7 загружен  ·  " .. S.KeyMenu .. " — меню  ·  "
    .. tostring(S.KeyPanic) .. " — паника", 5)
